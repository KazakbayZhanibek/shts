 ...
 *
 * ЗАЩИТА ТОКЕНОМ (обязательно):
 *  1. Слева внизу откройте «Настройки проекта» (шестерёнка) -> раздел
 *     «Свойства скрипта» -> «Добавить свойство»: имя APP_TOKEN,
 *     значение — длинное секретное слово (никому не показывайте, в код его писать НЕ надо).
 *  2. Разверните новую версию (см. ВАЖНО выше). Сайт спросит слово один раз
 *     и сохранит его в браузере. Без верного слова таблица не отвечает:
 *     чужой URL из публичного репозитория становится бесполезным.
 */
...
// Проверка кодового слова. Хранится в: Настройки проекта -> Свойства скрипта -> APP_TOKEN.
function tokenOk_(given) {
  const saved = PropertiesService.getScriptProperties().getProperty("APP_TOKEN");
  if (!saved) return { ok: false, reason: "token_not_configured" };
  if (String(given || "") !== saved) return { ok: false, reason: "forbidden" };
  return { ok: true };
}

// GET ?action=list&token=... -> {transactions, goals, deposits}
function doGet(e) {
  const check = tokenOk_(e && e.parameter && e.parameter.token);
  if (!check.ok) return json_({ error: check.reason });
  const out = {
...
// POST {action:"upsert"|"delete", sheet:"...", row:{...}, token:"..."}
function doPost(e) {
  let body = {};
  try { body = JSON.parse(e.postData.contents); } catch (err) {
    return json_({ ok: false, error: "bad json" });
  }
  const check = tokenOk_(body && body.token);
  if (!check.ok) return json_({ ok: false, error: check.reason });
